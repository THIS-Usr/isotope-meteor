...
//////////dlists /////////////
Template.dlists.loading = function () {
return !listsHandle.ready();
};

Template.dlists.lists = function () {
return Lists.find({}, {sort: {name: 1}});
};

Template.dlists.events({
'mousedown .list': function (evt) { // select list
Router.setList(this._id);
},
'click .list': function (evt) {
...
Template.dlists.events(okCancelEvents(
		  '#new-list',
		  {
		    ok: function (text, evt) {
		      var id = Lists.insert({name: text});
...
Template.dlists.selected = function () {
		  return Session.equals('list_id', this._id) ? 'selected' : '';
		};

Template.dlists.name_class = function () {
		  return this.name ? '' : 'empty';
		};

Template.dlists.editing = function () {
		  return Session.equals('editing_listname', this._id);
		};
...
Template.dtodos.loading = function () {
  return todosHandle && !todosHandle.ready();
};

Template.dtodos.any_list_selected = function () {
  return !Session.equals('list_id', null);
};

Template.dtodos.events(okCancelEvents(
  '#new-todo',
  {
    ok: function (text, evt) {
      var tag = Session.get('tag_filter');
...
Template.dtodos.todos = function () {
  // Determine which todos to display in main pane,
  // selected based on list_id and tag_filter.
...
Template.ditem.tag_objs = function () {
	  var todo_id = this._id;
	  return _.map(this.tags || [], function (tag) {
	    return {todo_id: todo_id, tag: tag};
	  });
...
Template.ditem.done_class = function () {
	  return this.done ? 'done' : '';
	};

Template.ditem.done_checkbox = function () {
	  return this.done ? 'checked="checked"' : '';
	};

Template.ditem.editing = function () {
	  return Session.equals('editing_itemname', this._id);
	};

Template.ditem.adding_tag = function () {
	  return Session.equals('editing_addtag', this._id);
	};

Template.ditem.events({
	  'click .check': function () {
	    Todos.update(this._id, {$set: {done: !this.done}});
	  },
...
Template.ditem.events(okCancelEvents(
	  '#todo-input',
	  {
	    ok: function (value) {
	      Todos.update(this._id, {$set: {text: value}});
